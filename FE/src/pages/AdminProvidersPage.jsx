...
function AdminProvidersPage() {
  ...
  const load = () => {
    adminApi.pendingProviders()
      ...
      .catch(() => setMessage('Không thể tải danh sách kho vận chờ duyệt.'));
  };
  ...
  return (
    <AdminShell title="Xác minh kho vận" subtitle="Duyệt tài khoản nhà cung cấp/kho vận trước khi họ quản lý sản phẩm và đơn hàng.">
      <section className="admin-panel admin-flow-explain">
        ...
        <ol>
          <li><strong>1. Ai là kho vận?</strong><span>Kho vận là nhân viên/đơn vị nội bộ phụ trách tạo sản phẩm, quản lý tồn kho và xử lý giao hàng.</span></li>
          ...
          <li><strong>3. Sau khi được cấp tài khoản</strong><span>Kho vận đăng nhập vào trang kho vận, tạo sản phẩm, cập nhật ảnh, giá, SKU và tồn kho.</span></li>
          ...
          <li><strong>5. Xử lý đơn</strong><span>Đơn đi qua: chờ xác nhận, đã xác nhận, đang chuẩn bị, đang giao, đã giao, hoàn tất.</span></li>
        </ol>
      </section>
      ...
      <section className="admin-panel">
        ...
        {providers.length === 0 && <p className="admin-empty">Không có kho vận chờ duyệt.</p>}
        <div className="admin-card-grid">
          {providers.map((provider) => (
            <article className="admin-mini-card" key={provider.id}>
              <span className="admin-mini-card__icon"><Truck size={22} /></span>
